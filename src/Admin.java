...
public class Admin extends Support {
    ...
    private void jobOpening() {
        job = new Job();
        if (jobList.size() >= 1) {
            num = Integer.parseInt(jobList.get(jobList.size() - 1).getJobCode().substring(3)) + 1;
        }
        ...
        try {
            for (int i = 0; i < jobList.size(); i++) {
                ...
                if (jobList.get(i).getJobCode().equals(jobCode)) {
                    ...
                    if (check) {
                        jobList.remove(job); //joblist에서 공고를 삭제합니다.
                        for (Member m : members) {
                            for (int k = 0; k < m.getAppliedJobCode().length; k++) {
                                if (m.getAppliedJobCode()[k].equals(jobCode)) { //jobCode라면
                                    m.setAppliedJobCodeIntoNull(k);
                                }
                            }
                        }
                    }
    ...
    //=========================================================================
    String[] removeElement(String[] arr, int index) {
        // https://www.delftstack.com/ko/howto/java/java-remove-element-from-array-and-shift/ <<참고
        
        String[] arrDestination = new String[arr.length];
    ...
    private void checkCandiList() {    //공고별 출력
        ...
        check = false;    //재할당
        for (Map.Entry<String, Member> entry : candiMap.entrySet()) {    //지원한 사람 출력
            for (int i = 0; i < 5; i++) {
                if (entry.getValue().getAppliedJobCode()[i].equals(findCode)) {
                    System.out.println(count + ". " + entry.getValue().getAllMember());
                    count++;
                    check = true;
                }
            }
